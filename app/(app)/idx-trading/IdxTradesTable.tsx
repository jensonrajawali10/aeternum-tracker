...
export function IdxTradesTable() {
  const { data, isLoading } = useSWR<{ trades: Trade[] }>("/api/journal/trades?book=idx_trading&limit=50", fetcher, {
  ...
  if (!trades.length)
    return (
      <div className="text-[11px] text-muted">
        No IDX trades yet. Generate the Apps Script below and send it to your trader.
      </div>
    );
        ...
        <tbody>
          {trades.map((t) => (
            <tr key={t.id} className="border-b border-border hover:bg-hover">
              ...
              <td>
                <span className={clsx("text-[9.5px] px-1.5 py-0.5 rounded border uppercase tracking-wide", t.direction === "LONG" ? "text-green border-green/30" : "text-red border-red/30")}>
                  {t.direction}
                </span>
              </td>
              <td className="text-muted">{t.strategy ?? "—"}</td>
              <td className="text-right">{fmtNumber(t.entry_price, 0)}</td>
              <td className="text-right">{t.exit_price != null ? fmtNumber(t.exit_price, 0) : "—"}</td>
              ...
              <td className={clsx("text-right", signClass(t.pnl_pct))}>{fmtPct(t.pnl_pct, 2, true)}</td>
              <td className="text-right text-[10px] uppercase tracking-wide text-muted">{t.result ?? "open"}</td>
            </tr>
          ))}
        </tbody>
      </table>
